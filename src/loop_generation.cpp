...
void writeLoops(char* pdbselect, char* loopfile, char* gridfile, const vec_3D& grid){
  ...
  write_progress(500, std::vector<std::vector<std::vector<int> > >(500, std::vector<std::vector<int > >(10, std::vector<int>(3,0) ) ) );
  ...
  if (!infile){
    std::cerr << "Can't open " << infile << " to read." << std::endl;
    return;
  }
...
void db_query(float dCA, float dCB, int loop_length, char* pdbselect, char* loopfile, char* gridfile){
  ...
  int number_of_loops;
  int record_start;
  grid_in.seekg( (40000 * dCA) + (80 * dCB) + (8 * (loop_length - 1) ) );
